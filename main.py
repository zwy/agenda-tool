...
def get_task_files():
    ...
    if not tasks_dir.exists():
        logger.error(f"任务文件夹 {tasks_dir} 不存在")
        return []
...
def is_process_running(pid):
    ...
    if system == 'Windows':
        try:
            ...
            logger.warning("psutil 库未安装，无法准确检查 Windows 上的进程状态")
            return False
...
def main():
    ...
    if os.path.exists(lock_file_path):
        ...
        if pid and is_process_running(pid):
            logger.warning("脚本已经在运行，退出。")
            sys.exit(0)
        else:
            # PID对应的进程不存在或无效，清理锁文件和PID文件
            logger.info("发现残留锁文件，但原进程已不存在，清理锁文件和PID文件。")
        ...
        for task_file in task_files:
            ...
            for attempt in range(1, 4):
                try:
                    ...
                    # 记录脚本的标准输出和标准错误
                    if process.stdout:
                        logger.info(f"任务 {task_name} Print:\n{process.stdout}")
                    if process.stderr:
                        logger.info(f"任务 {task_name} Logger:\n{process.stderr}")
                    ...
                    if process.returncode == 0:
                        ...
                        logger.error(f"任务 {task_name} 执行失败，返回码: {process.returncode}，第{attempt}次尝试")
                        if attempt < 3:
                            logger.info(f"等待1秒后重试任务 {task_name}...")
                            time.sleep(1)
                        else:
                            logger.error(f"任务 {task_name} 达到最大重试次数，停止执行")
            ...
            if has_error:
                logger.error(f"检测到日志中存在错误信息 ({error_pattern})，停止后续任务执行")
                break
