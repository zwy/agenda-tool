...
def process_bibao_data():
    ...
    if not bibao_file.exists():
        logger.error(f"错误: 文件 {bibao_file} 不存在!")
        return
    
    if not video_file.exists():
        logger.error(f"错误: 文件 {video_file} 不存在!")
        return
    ...
    try:
        ...
        logger.info(f"已读取{len(bibao_list)}条壁报数据")
    except Exception as e:
        logger.error(f"读取壁报数据时出错: {str(e)}")
        return
    ...
    try:
        ...
        logger.info(f"已读取{len(video_dict)}条视频数据")
    except Exception as e:
    ...
    for bibao in bibao_list:
        if "aliyunVid" in bibao and bibao["aliyunVid"]:
            ...
            if aliyun_vid in video_dict:
                ...
                logger.error(f"错误: 壁报的aliyunVid={aliyun_vid}在视频数据中找不到对应记录")
                error_count += 1

    logger.info(f"已更新{updated_count}条壁报的视频相关信息")
    if error_count > 0:
        logger.error(f"有{error_count}条壁报数据出现错误，请检查")
        return
    ...
    for bibao in bibao_list:
        if "aliyunVid" in bibao and bibao["aliyunVid"]:
            if "permission" not in bibao or "duration" not in bibao:
                logger.error(f"错误: 壁报数据aliyunVid={bibao['aliyunVid']}没有完整的视频信息")
                return
    ...
    try:
        ...
        logger.info(f"已将更新后的数据保存到 {bibao_file}")
    except Exception as e:
