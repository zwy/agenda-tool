...
def upload_file_to_oss(local_file, remote_path):
    ...
    try:
        ...
        if result.status == 200:
            ...
            logging.error(f"上传失败: {local_file} -> {remote_path}, 状态码: {result.status}")
            return False
    except Exception as e:
        logging.error(f"上传出错: {local_file} -> {remote_path}, 错误: {str(e)}")
        return False
...
def sync_files_to_oss():
    ...
    if not LOCAL_DATA_DIR.exists():
        logging.warning(f"本地数据目录不存在: {LOCAL_DATA_DIR}")
        return
    
    if not OSS_DATA_DIR:
        logging.error("OSS_DATA_DIR 环境变量未设置")
        return
    
    logging.info(f"开始同步文件到OSS，本地目录: {LOCAL_DATA_DIR}")
    logging.info(f"目标OSS路径: {OSS_DATA_DIR}")
    ...
    for file_path in LOCAL_DATA_DIR.glob('**/*'):
        ...
        if file_path.is_file():
            ...
            logging.info(f"正在上传: {rel_path} -> {oss_path}")
            if upload_file_to_oss(str(file_path), oss_path):
                logging.info(f"上传成功: {rel_path}")
                uploaded_files += 1
            else:
                logging.error(f"上传失败: {rel_path}")
            
            # 每上传一个文件暂停一小段时间，避免请求过于频繁
            time.sleep(0.2)  # 使用普通的time.sleep代替asyncio.sleep
    
    logging.info(f"文件同步完成。总文件数: {total_files}, 成功上传: {uploaded_files}")

# 修改为同步主函数
def main():
    """主函数"""
    logging.info("开始执行OSS同步任务")
    sync_files_to_oss()
    logging.info("OSS同步任务完成")
